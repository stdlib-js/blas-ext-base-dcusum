...
tape( 'the function has an arity of 8', function test( t ) {
	t.strictEqual( dcusum.length, 8, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the cumulative sum', function test( t ) {
	...
	dcusum( x.length, 10.0, x, 1, 0, y, 1, 0 );
	expected = new Float64Array([
		11.0,
		13.0,
		16.0,
...
tape( 'the function returns a reference to the output array', function test( t ) {
	...
	out = dcusum( x.length, 0.0, x, 1, 0, y, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', function test( t ) {
	...
	dcusum( -1, 0.0, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );

	dcusum( 0, 0.0, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );
...
tape( 'the function supports negative strides', function test( t ) {
	...
	dcusum( 3, 0.0, x, -2, x.length-1, y, -1, 2 );
